...
export default function CalculationScreen() {
  ...
  const calculateTotal = () => {
    ...
    else if (selected.length === 3) rate = 0.1;
    ...
    // results saved into the state here
    setSubtotal(sub);
  ...
  const makeBooking = () => {
    ...
    if (!name || !email || !phone || selected.length < 1) {
      Alert.alert(
        "Missing",
        "Fill in contact details and choose at least 1 course"
      );
      return;
    }
    ...
    Alert.alert("Booked", "We will contact you soon");
    ...
    <ScrollView style={styles.container}>
      ...
      {subtotal > 0 && (
        <>
          ...
          <Text style={styles.summaryLine}>
            Discount ({discountRate * 100}%): -R{discount.toFixed(2)}
          </Text>
